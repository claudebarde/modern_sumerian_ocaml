...
function check_same_length(f, param, length) {
  const length_a = param.length;
  if (length !== length_a) {
    return Curry._3(Stdlib__Printf.ksprintf(Stdlib.invalid_arg, {
      TAG: /* Format */ 0,
      _0: {
...
function get(param, i) {
  const v = Caml_array.get(param.arr, i);
  if (v === param.dummy) {
    let f = "get";
...
function find_last(param) {
  ...
  const i = length - 1 | 0;
  const v = arr[i];
  return Caml_option.some(v === param.dummy ? missing_element(i, length) : v);
}

function pop_last(param) {
  const length = param.length;
  const arr = param.arr;
  const dummy = param.dummy;
  const capacity = arr.length;
  if (length > capacity) {
    invalid_length(length, capacity);
  }
...
function remove_last(param) {
  const last = param.length - 1 | 0;
  if (last >= 0) {
    param.length = last;
    return Caml_array.set(param.arr, last, param.dummy);
  }
  
}

function truncate(param, n) {
  if (n < 0) {
  ...
  if (length <= n) {
    ...
    param.length = n;
    return Stdlib__Array.fill(arr, n, length - n | 0, param.dummy);
  }
}
...
function ensure_capacity(param, capacity_request) {
  const arr = param.arr;
  ...
  const new_arr = Curry._4(Dummy_Array.extend, arr, param.length, param.dummy, new_capacity);
  param.arr = new_arr;
...
function fit_capacity(param) {
  if (param.arr.length === param.length) {
    return;
  } else {
    param.arr = Curry._2(Dummy_Array.prefix, param.arr, param.length);
    return;
  }
}

function set_capacity(param, n) {
  if (n < 0) {
  ...
  const cur_capacity = arr.length;
  if (n < cur_capacity) {
    param.length = param.length < n ? param.length : n;
    param.arr = Curry._2(Dummy_Array.prefix, arr, n);
    return;
  } else if (n > cur_capacity) {
    param.arr = Curry._4(Dummy_Array.extend, arr, param.length, param.dummy, n);
    return;
  } else {
    return;
  }
...
function blit_assume_room(param, src_pos, src_length, param$1, dst_pos, dst_length, blit_length) {
  const src_arr = param.arr;
  const dst_arr = param$1.arr;
  check_same_length("blit", param, src_length);
  check_same_length("blit", param$1, dst_length);
  if ((dst_pos + blit_length | 0) > dst_length) {
    param$1.length = dst_pos + blit_length | 0;
  }
  Curry._7(Dummy_Array.blit, src_arr, param.dummy, src_pos, dst_arr, param$1.dummy, dst_pos, blit_length);
}
...
function append_array_if_room(param, b) {
  const length_a = param.length;
  const arr = param.arr;
  ...
  if ((length_a + length_b | 0) > arr.length) {
    ...
    param.length = length_a + length_b | 0;
...
function append_if_room(param, b, length_b) {
  const length_a = param.length;
  const arr_a = param.arr;
  if ((length_a + length_b | 0) > arr_a.length) {
    return false;
  } else {
    blit_assume_room(b, 0, length_b, param, length_a, length_a, length_b);
